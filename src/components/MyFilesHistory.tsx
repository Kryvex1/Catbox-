...
export const MyFilesHistory: React.FC<MyFilesHistoryProps> = ({
  ...
  const getCategory = (file: FileRecord) => {
    ...
    if (['png', 'jpg', 'jpeg', 'gif', 'webp', 'svg'].includes(ext) || file.mimetype.startsWith('image/')) return 'image';
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500 dark:text-gray-400">
                {files.length} total files hosted on this Catbox instance
              </p>
            </div>
          </div>
          ...
          {selectedFilenames.length > 0 && (
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className="px-3 py-1.5 text-xs font-semibold rounded-xl bg-sky-600 hover:bg-sky-700 text-white flex items-center gap-1.5 shadow-xs transition-colors"
              >
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full pl-9 pr-4 py-2 text-xs rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-white focus:ring-2 focus:ring-sky-500 focus:outline-hidden"
            />
          </div>

          <div className="flex items-center gap-1 overflow-x-auto bg-gray-100 dark:bg-gray-800 p-1 rounded-xl border border-gray-200 dark:border-gray-700 text-xs">
      ...
      {filteredFiles.length === 0 ? (
        <div className="bg-white dark:bg-[#1e2327] rounded-3xl p-12 text-center border border-gray-200 dark:border-gray-800 space-y-3">
          ...
          <p className="text-xs text-gray-400">
            {searchTerm ? 'Try searching with a different keyword.' : 'Upload your first file using the Upload tab above!'}
          </p>
        </div>
      ) : (
        <div className="bg-white dark:bg-[#1e2327] rounded-3xl border border-gray-200 dark:border-gray-800 overflow-hidden shadow-sm">
          <div className="px-6 py-3 border-b border-gray-200 dark:border-gray-800 flex items-center justify-between text-xs font-semibold text-gray-500 dark:text-gray-400 bg-gray-50/50 dark:bg-gray-900/30">
            <div className="flex items-center gap-3">
              <input
                type="checkbox"
          ...
          <div className="divide-y divide-gray-100 dark:divide-gray-800/60">
            {filteredFiles.map((file) => {
              ...
              return (
                <div
                  key={file.id}
                  className={`p-4 flex items-center justify-between gap-4 hover:bg-gray-50/70 dark:hover:bg-gray-800/40 transition-colors ${
                ...
                >
                  <div className="flex items-center gap-3 min-w-0 flex-1">
                    ...
                    <div 
                      ...
                      className="w-12 h-12 rounded-xl bg-gray-100 dark:bg-gray-800 overflow-hidden flex items-center justify-center flex-shrink-0 cursor-pointer border border-gray-200 dark:border-gray-700 hover:opacity-80 transition-opacity"
                    >
                      {cat === 'image' ? (
                        <img
                          src={file.url}
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2">
                        <p
                          onClick={() => onPreviewFile(file)}
                          className="text-xs font-semibold text-gray-900 dark:text-gray-100 truncate hover:text-sky-600 dark:hover:text-sky-400 cursor-pointer"
                          title={file.originalName || file.filename}
                        >
                          {file.originalName || file.filename}
                        </p>
                        {file.mode === 'litterbox' && (
                          <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded bg-amber-100 dark:bg-amber-950 text-amber-700 dark:text-amber-300 flex items-center gap-0.5">
